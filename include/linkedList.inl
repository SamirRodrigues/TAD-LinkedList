...
{	
	 ...
	 * To do throw std::out_of_range("The list is empty!");
	 * @tparam T 
	 * @return const T& 
	 */
	template <typename T>
	...
	{
		typename list<T>::Node* temp = new typename list<T>::Node(value,itr.current->prev,itr.current);
		length++;
	...
	{			
		auto ref = list<T>::iterator(pos.current->next); 
		if(pos != end())
		{
